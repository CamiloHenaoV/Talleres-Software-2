...
public class UserService {
     ...
     * @return Resultado de la operación
     */
    public ServiceResult changePassword(Integer userId, String oldPassword, String newPassword) {
        ...
        List<String> passwordErrors = validator.validatePassword(newPassword);
        if (!passwordErrors.isEmpty()) {
            return ServiceResult.failure(String.join(", ", passwordErrors));
        }
        ...
        if (updated) {
            return ServiceResult.success("Contraseña actualizada exitosamente");
        } else {
            return ServiceResult.failure("Error al actualizar la contraseña");
        }
    }
     ...
     * @return Resultado de la operación
     */
    public ServiceResult setUserActiveStatus(Integer userId, boolean active) {
        Optional<User> userOptional = repository.findById(userId);
        if (!userOptional.isPresent()) {
            return ServiceResult.failure("Usuario no encontrado");
        }
        ...
        if (updated) {
            ...
            return ServiceResult.success("Usuario " + status + " exitosamente");
        } else {
            return ServiceResult.failure("Error al cambiar el estado del usuario");
        }
    }
